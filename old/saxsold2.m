...
for T=Trange
    col = (T-1)/(Trange(end)-1);
    
    plot(q,s(:,T),'color',[col 0 1-col]);
end
plot([qmin0,qmin0],[5,5e2],'k-','linewidth',3)
plot([qmin0f,qmin0f],[5,5e2],'k-','linewidth',3)

xlabel('R_Mq');ylabel('S(q)')
set(gca,'xscale','log');set(gca,'yscale','log');
xlim([0.02,0.6]);ylim([5,2e2]);box on
%% 
